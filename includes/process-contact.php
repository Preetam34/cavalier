	...
	if ($_POST['action'] == 'add') {
		...
		if ($email != '' && $message != '') {
			if (preg_match($pattern, $email)) {
				if (!empty($name)) $subject = 'Message from ' . $name;
				else $subject = 'Message from Guest';
				$headers = 'From: ' . $email . "\r\n";
				$mail = mail($my_email, $subject, $message, $headers);

				if ($mail) echo 'success|Send a message process completed.';
